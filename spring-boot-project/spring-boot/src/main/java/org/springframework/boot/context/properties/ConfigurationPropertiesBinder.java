...
class ConfigurationPropertiesBinder {
	...
	BindResult<?> bind(ConfigurationPropertiesBean propertiesBean) {
		...
		ConfigurationProperties annotation = propertiesBean.getAnnotation();      //获取注解信息
		BindHandler bindHandler = getBindHandler(target, annotation);        //根据注解上不同的配置，使用不同的handler
		return getBinder().bind(annotation.prefix(), target, bindHandler);    //getBinder，获取到前边绑定的binder:持有propertiesSourcesList
	}
