...
public class Binder {
	...
	private <T> T bind(ConfigurationPropertyName name, Bindable<T> target, BindHandler handler, Context context,
		...
		try {
			...
			Object bound = bindObject(name, target, handler, context, allowRecursiveBinding);   //bindObject
	...
	private ConfigurationProperty findProperty(ConfigurationPropertyName name, Context context) {
		...
		//按顺序遍历source，直到查找到属性，so，确实propertiesSourcesList的顺序即配置文件的优先级
		//StreamSupport.stream(context.getSources().spliterator(), false).collect(Collectors.toList());
