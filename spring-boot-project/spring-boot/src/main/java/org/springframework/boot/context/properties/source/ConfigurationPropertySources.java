...
public final class ConfigurationPropertySources {
	 ...
	 * @see #get(Environment)
	 *
	 * 附加一个 ConfigurationPropertySource 到指定的 Environment.
	 * 调整每个由环境管理的 PropertySource 为 ConfigurationPropertySource，
	 * 并允许 PropertySourcesPropertyResolver调用使用 property names调用         //?????
	 *
	 * 附加的解析程序将动态跟踪从基础 Environment 属性源添加或删除的任何内容
	 *
	 */
	public static void attach(Environment environment) {
		Assert.isInstanceOf(ConfigurableEnvironment.class, environment);
		//前两步配置的PropertySource
		...
		if (attached == null) {
			//将前面读取的配置统一交给configurationProperties，看ConfigurationPropertySourcesPropertySource的代码得知这就是一个方便查找配置的工具类
