...
class BeanDefinitionLoader {
	...
	private int load(Object source) {
		...
		if (source instanceof Resource) {   //xml
			return load((Resource) source);
		}
		if (source instanceof Package) {
			//scan, 跟spring处理@componentScan一样，这部分bd提前加载了
			return load((Package) source);
		}
		if (source instanceof CharSequence) {  //xml
	...
	private int load(Class<?> source) {
		if (isGroovyPresent() && GroovyBeanDefinitionSource.class.isAssignableFrom(source)) {  //Groovy相关，忽略
			// Any GroovyLoaders added in beans{} DSL can contribute beans here
			GroovyBeanDefinitionSource loader = BeanUtils.instantiateClass(source, GroovyBeanDefinitionSource.class);
			load(loader);
		}
		if (isComponent(source)) {    //判断有没有Component注解
			//先把source的bd注册到bdmap中
			//spring启动-》parse这个BD-》componentScan-》当前package所有bean
